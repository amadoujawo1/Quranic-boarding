...
// ── 12-Month Calendar Month Selector Component ──────────────────────────────
const CALENDAR_MONTHS = [
  { num: 1, key: 'January', short: 'Jan', term: 'Term 2', season: 'Q1' },
  { num: 2, key: 'February', short: 'Feb', term: 'Term 2', season: 'Q1' },
  { num: 3, key: 'March', short: 'Mar', term: 'Term 2', season: 'Q1' },
  { num: 4, key: 'April', short: 'Apr', term: 'Term 2', season: 'Q1' },
  { num: 5, key: 'May', short: 'May', term: 'Term 3', season: 'Q2' },
  { num: 6, key: 'June', short: 'Jun', term: 'Term 3', season: 'Q2' },
  { num: 7, key: 'July', short: 'Jul', term: 'Term 3', season: 'Q2' },
  { num: 8, key: 'August', short: 'Aug', term: 'Term 3', season: 'Q2' },
  { num: 9, key: 'September', short: 'Sep', term: 'Term 1', season: 'Q3' },
  { num: 10, key: 'October', short: 'Oct', term: 'Term 1', season: 'Q3' },
  { num: 11, key: 'November', short: 'Nov', term: 'Term 1', season: 'Q3' },
  { num: 12, key: 'December', short: 'Dec', term: 'Term 1', season: 'Q4' },
];

interface YearCalendarMonthPickerProps {
  ...
  onSelectMultipleMonths?: (months: string[]) => void;
  feePerMonth?: number;
}

const YearCalendarMonthPicker: React.FC<YearCalendarMonthPickerProps> = ({
  mode,
  year,
  onYearChange,
  selectedMonth,
  ...
  selectedMonths = [],
  onToggleMonth,
  onSelectMultipleMonths,
  feePerMonth
}) => {
  const currentYear = year || 2026;

  const handlePreset = (preset: 'all' | 'term1' | 'term2' | 'term3' | 'clear') => {
    if (!onSelectMultipleMonths) return;
    if (preset === 'clear') {
      onSelectMultipleMonths([]);
      return;
    }
    if (preset === 'all') {
      const all = CALENDAR_MONTHS.map(m => `${m.key} ${currentYear}`);
      onSelectMultipleMonths(all);
      return;
    }
    if (preset === 'term1') {
      const t1 = ['September', 'October', 'November', 'December'].map(m => `${m} ${currentYear}`);
      onSelectMultipleMonths(t1);
      return;
    }
    if (preset === 'term2') {
      const t2 = ['January', 'February', 'March', 'April'].map(m => `${m} ${currentYear}`);
      onSelectMultipleMonths(t2);
      return;
    }
    if (preset === 'term3') {
      const t3 = ['May', 'June', 'July', 'August'].map(m => `${m} ${currentYear}`);
      onSelectMultipleMonths(t3);
      return;
    }
  };

  return (
    <div className="bg-slate-50 dark:bg-slate-800/80 rounded-2xl p-4 border border-slate-200 dark:border-slate-700/80 space-y-3">
      {/* Calendar Header with Year Controls & Presets */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 pb-2 border-b border-slate-200 dark:border-slate-700">
        <div className="flex items-center gap-2">
          <Calendar className="w-4 h-4 text-gold-500" />
          <span className="text-xs font-bold text-slate-800 dark:text-slate-200">
            {mode === 'multiple' ? 'Full-Year Calendar Month Selector' : 'Select Payment Month'}
          </span>
          {mode === 'multiple' && (
            <span className="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-300 border border-blue-300 dark:border-blue-800">
        ...
        {/* Year Switcher */}
        <div className="flex items-center gap-1.5 bg-white dark:bg-slate-900 px-2 py-1 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-bold shadow-xs">
          <button
            type="button"
            onClick={() => onYearChange(currentYear - 1)}
            className="px-1.5 py-0.5 rounded hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 hover:text-slate-900 dark:hover:text-white transition cursor-pointer"
            title="Previous Year"
          >
            ◀
          </button>
          <span className="px-2 text-gold-600 dark:text-gold-400 font-extrabold tracking-wide">
      ...
      {mode === 'multiple' && (
        <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
          ...
          <button
            ...
            className="px-2.5 py-1 text-[10px] font-bold rounded-lg bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 hover:bg-emerald-200 border border-emerald-300 dark:border-emerald-800 transition cursor-pointer"
          >
            ⚡ Full Year (All 12 Months)
          </button>
          <button
            type="button"
            onClick={() => handlePreset('term1')}
            className="px-2.5 py-1 text-[10px] font-bold rounded-lg bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200 hover:bg-slate-300 transition cursor-pointer"
          >
            Term 1 (Sep - Dec)
          </button>
          <button
            type="button"
            onClick={() => handlePreset('term2')}
            className="px-2.5 py-1 text-[10px] font-bold rounded-lg bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200 hover:bg-slate-300 transition cursor-pointer"
          >
            Term 2 (Jan - Apr)
          </button>
          ...
          <button
            ...
            className="px-2 py-1 text-[10px] font-semibold rounded-lg text-slate-500 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40 transition cursor-pointer ml-auto"
          >
            Clear
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
        {CALENDAR_MONTHS.map((m) => {
          const monthString = `${m.key} ${currentYear}`;
          const isSelected =
            mode === 'multiple'
              ? selectedMonths.includes(monthString)
          ...
          return (
            <button
              ...
              onClick={() => {
                if (mode === 'multiple') {
                  if (onToggleMonth) onToggleMonth(monthString);
                } else {
                  if (onSelectMonth) onSelectMonth(monthString);
                }
              }}
              className={`p-2.5 rounded-xl border text-left transition-all relative flex flex-col justify-between cursor-pointer ${
                isSelected
                  ? 'bg-gradient-to-br from-emerald-900 to-emerald-950 border-gold-500 text-white shadow-md ring-2 ring-gold-500/40 scale-[1.02]'
                  : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700/80 text-slate-700 dark:text-slate-300 hover:border-gold-500/60 hover:bg-slate-50 dark:hover:bg-slate-800/60'
              }`}
            >
              <div className="flex justify-between items-start">
                <span className={`text-[10px] font-extrabold tracking-wider uppercase ${isSelected ? 'text-gold-400' : 'text-slate-400'}`}>
                  {m.short}
                </span>
                <span className={`text-[9px] px-1.5 py-0.5 rounded font-semibold ${
                  isSelected ? 'bg-gold-500/20 text-gold-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'
                }`}>
                  {m.term}
                </span>
              </div>
              ...
              <div className="mt-1">
                <div className={`text-xs font-bold leading-tight ${isSelected ? 'text-white font-black' : 'text-slate-900 dark:text-white'}`}>
                  {m.key}
                </div>
              ...
              <div className="mt-2 pt-1 border-t border-slate-100 dark:border-slate-800/60 flex items-center justify-between">
                <span className={`text-[9px] font-medium ${isSelected ? 'text-gold-400 font-bold' : 'text-slate-400'}`}>
                  {feePerMonth ? `GMD ${feePerMonth.toLocaleString()}` : isSelected ? 'Selected' : 'Available'}
                </span>
                {isSelected ? (
...
export const FinancePage: React.FC = () => {
  ...
  const [multiCalendarYear, setMultiCalendarYear] = useState<number>(2026);
  const [paymentModalYear, setPaymentModalYear] = useState<number>(2026);
  const [batchCalendarYear, setBatchCalendarYear] = useState<number>(2026);


  const handleToggleMultiMonth = (monthString: string) => {
    if (multiSelectedMonths.includes(monthString)) {
      setMultiSelectedMonths(multiSelectedMonths.filter(m => m !== monthString));
    } else {
      setMultiSelectedMonths([...multiSelectedMonths, monthString]);
    }
  };
      ...
      {showGenerateDuesModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                Target Month <span className="text-rose-500">*</span>: <span className="font-extrabold text-gold-600 dark:text-gold-400">{batchMonth}</span>
              </label>
              <YearCalendarMonthPicker
                mode="single"
                year={batchCalendarYear}
                onYearChange={setBatchCalendarYear}
                selectedMonth={batchMonth}
                onSelectMonth={(m) => setBatchMonth(m)}
              />
            </div>
          ...
          >
            ...
            {multiMonthSuccess.length === 0 && (
              <>
                ...
                {/* Full-Year Calendar Month Selector */}
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5 flex items-center justify-between">
                    <span>
                      Select Payment Months <span className="text-rose-500">*</span>
                    </span>
                    <span className="text-blue-600 dark:text-blue-400 font-extrabold text-xs">
                      {multiSelectedMonths.length} month(s) selected
                    </span>
                  </label>
                  <YearCalendarMonthPicker
                    mode="multiple"
                    year={multiCalendarYear}
                    onYearChange={setMultiCalendarYear}
                    selectedMonths={multiSelectedMonths}
                    onToggleMonth={handleToggleMultiMonth}
                    onSelectMultipleMonths={setMultiSelectedMonths}
                    feePerMonth={multiFeePerMonth}
                  />

                  {multiSelectedMonths.length > 0 && (
                    <div className="mt-2.5 p-2.5 bg-blue-50/80 dark:bg-blue-950/40 rounded-xl border border-blue-200 dark:border-blue-800/60 flex flex-wrap items-center justify-between gap-2">
                      <div className="text-[11px] text-slate-700 dark:text-slate-300">
                        <span className="font-bold text-blue-700 dark:text-blue-400">Selected Months ({multiSelectedMonths.length}):</span>{' '}
                        <span className="font-semibold">{multiSelectedMonths.join(', ')}</span>
                      </div>
                      <button
                        type="button"
                        onClick={() => setMultiSelectedMonths([])}
                        className="text-[10px] text-rose-500 hover:text-rose-700 font-bold underline cursor-pointer ml-auto"
                      >
                        Reset Months
                      </button>
                    </div>
